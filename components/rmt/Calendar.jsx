...
const Calendar = ({ appointments }) => {
  ...
  // Paginated slice
  const indexOfLast = page * appointmentsPerPage;
  ...
  const formatAppointmentDate = (appointment) => {
    try {
      ...
      return `${formattedDate} at ${displayHours}:${minutes
        .toString()
        .padStart(2, "0")} ${ampm}`;
    } catch {
      return "Invalid date";
  ...
  const isHealthHistoryOutdated = (lastUpdate) => {
  ...
  return (
    <div>
      ...
      {upcomingAppointments.length === 0 ? (
        ...
        <>
          {/* All appointments together */}
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
            {currentAppointments.map((appointment) => (
              <Link
                key={appointment.id}
                href={`/dashboard/rmt/client-profile/${appointment.clientId}`}
              ...
              >
                ...
                {appointment.consentForm && (
                  <div className="mt-3 mb-2">
                    ...
                    {getAreasToAvoid(appointment.consentForm).length > 0 && (
                      <>
                ...
                >
                  ...
                  <button
                    onClick={() =>
                      router.push(
                        `/dashboard/rmt/reschedule-appointment/${appointment.id}`
                      )
                    }
                    disabled={loading}
                    className="
          ...
          <div className="mt-6 flex justify-between items-center">
            ...
            <span className="text-gray-600">
              ...
              {Math.min(indexOfLast, upcomingAppointments.length)} of{" "}
...
//   const isHealthHistoryOutdated = (lastUpdate) => {
//     if (!lastUpdate) return true; // No update recorded means outdated
...
//                       key={appointment.id}
//                       href={`/dashboard/rmt/client-profile/${appointment.clientId}`}
//                       className={`shadow-md rounded-lg p-4 hover:shadow-lg transition-shadow duration-200 cursor-pointer ${
//                         appointment.code
...
//                         <div className="flex flex-wrap gap-2 mb-2">
//                           {appointment.code && (
//                             <div className="inline-block bg-red-600 text-white text-xs font-semibold px-2 py-1 rounded">
//                               Gift Card
//                             </div>
//                           )}
//                           {isHealthHistoryOutdated(
...
//                           onClick={(e) => {
//                             e.preventDefault();
//                             setConfirmClear(appointment.id);
//                           }}
//                           disabled={loading}
